...
- **Detalle de difusión (clic en el mensaje):** modal con **📤 el mensaje FINAL que se envía** (`full_text`: ya sin ubicación/teléfonos, con markup y footer — lo mismo que sale/está saliendo) y **💰 comparador de precios** (anterior→nuevo por producto). El comparador se calcula al capturar (`componer_con_desglose` → `desglosar_precios`, empareja los precios 1:1 sobre el texto limpio) y se guarda como `price_diff` en el job; solo aparece en difusiones NUEVAS del canal (las capturas previas a la función no tienen el precio original guardado). El envío MANUAL va crudo → sin comparador.
