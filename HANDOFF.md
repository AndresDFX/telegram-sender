...
python scripts/verificar_deploy_render.py --esperar 900   # espera al despliegue tras un push
```
...
- **🏠 Inicio:** KPIs de 30 días (clicables: enviados, tasa, **por enviar**, **atascados**) + mini-gráfico + **sala de control**: switches «Capturar listas del canal» y «Envíos automáticos activos», selector **«Lista del envío automático»** por canal, card **«Última lista capturada»** (con «Enviar a…») y checklist de primeros pasos. Aquí viven los interruptores del negocio (antes enterrados en Ajustes).
- **✍️ Enviar:** compositor **ÚNICO**: texto + imagen + canales + «Enviar a» (listas/contactos) + previsualización + contador (límite 4096) + selector **«¿Cuándo se envía?»** con modos **⚡ Ahora / 📅 Una vez / 🔁 Se repite** (diario/semanal). El botón cambia a «Programar» según el modo. No hay formulario de programación aparte.
...
- **Ritmo vertical / espaciado (auditoría de espaciado):** regla general **`.card > * + button{margin-top:14px}`** (y `.card > * + .markup`) que da aire a cualquier botón de acción que siga a un bloque hermano DIRECTO de la card (Guardar…, etc.) sin quedar pegado al campo/opciones de arriba. Usa hijo directo (`>`) a propósito: NO afecta a botones dentro de filas horizontales/toolbars (`Crear lista` input+botón, `.compose-actions`, `.tbl-toolbar`), que conservan su alineación. Además: `.section-label` con más separación superior (margin-top 18px) para marcar sub-secciones; `.chan-row` sin el doble margen con el label; barra de acción de cuenta Telegram y botón «Vaciar cola» de la tira con espaciado/padding homogéneos. Base: workflow de auditoría de espaciado (22 hallazgos).
...
- **👥 Contactos** (sub-nav Telegram / WhatsApp): **Contactos** con buscador + paginación, filtro Todos / Incluidos / Excluidos y contador (pastillas «Excluido por nombre / Incluido a mano»), **Listas de contactos** (¿a quién enviar?: todos / solo las marcadas / todos excepto) y **«⛔ Excluir si el nombre contiene…»** (config: `*_exclude_patterns`).
- **⚙️ Ajustes** (sub-nav): **🔌 Conexiones** (cuenta de Telegram por bot o «mi cuenta personal», WhatsApp reenvío + **vinculación guiada por código desde el mismo teléfono** con el QR como alternativa, «Conectar bot») · **📥 Captura** (canal de Telegram, markup, símbolos, «texto al final del mensaje», «textos a eliminar de cada lista», imagen, «probar cómo queda una lista») · **📤 Ritmo y horarios** (anti-baneo «mensajes por grupo»/«pausa entre mensajes», ventanas, **zona horaria como select legible**) · **👤 Acceso** (usuarios con roles, correo de recuperación, cambio de contraseña) · **🛠️ Auditoría**.
...
- **Envío puntual:** ✍️ Enviar → texto + imagen → canales (ninguno preseleccionado) → «Enviar a» (listas/contactos) → modo ⚡ Ahora (o 📅 Una vez) → confirmar (muestra N destinatarios) → seguir en 📡 Actividad → Historial.
...
### CRUD por entidad y nomenclatura de acciones (agosto 2026)

**Matriz completa entidad × C/R/U/D → `docs/PLAN_PRUEBAS.md` §2.1.** Resumen de lo que se cerró y de lo que falta a propósito:

- **Lo que faltaba y se añadió:**
  - **Editar un programado** — `POST /api/schedules/update` + botón ✏️ en 📡 Actividad → Programados. Antes había que borrar y volver a crear (se perdía el historial de ejecuciones). Toda la validación vive en el helper de módulo **`_campos_schedule(c)` → `(campos, error)`**, que **comparten crear y editar**: si alguien añade una regla nueva, no puede olvidarse de un lado. `update` parte del schedule existente (`{**actual, **cambios}`), así que el panel manda solo lo que cambió, y **no toca `runs`/`last_run`** (no están entre los campos que devuelve el helper). Para `type=once`, `run_at` ausente se hereda de `next_run`.
  - **Editar un usuario** — `POST /api/users/update` (solo admin, 403 si no) cambia el **correo** y **restablece la contraseña** sin pedir la actual (el caso «se le olvidó»; el auto-servicio sigue siendo `auth/change-password`). Audita `users:actualizar` con *qué* campos se tocaron, **nunca el valor**. `GET /api/users` ahora devuelve **`principal`** (`admin_user()`): el panel ya no asumía que el admin principal se llama `admin` (lo tenía hardcodeado en dos sitios).
  - **Renombrar una lista de contactos** — ✏️ en 👥 Contactos. Las listas viven dentro de la config, así que renombrar es reescribirla; lo importante son las **referencias**: el rename lleva `auto_<canal>_list` en el **MISMO** `POST /api/config` (si no, quedaría apuntando al nombre viejo → el envío automático deja de difundir, variante de M25) y luego parchea con `/api/schedules/update` cada programado que usaba el nombre viejo, avisando antes de cuántos son. Rechaza duplicados y restaura el nombre local si el guardado falla.
  - **Reincluir un solo auto-excluido de WhatsApp** — `POST /api/whatsapp/blocked/clear {id}` (antes solo existía «todos»). Requiere el **servicio Node redesplegado en Render**; con la versión anterior el `{id}` se ignora y limpia todos los contadores (superconjunto inocuo, no rompe nada).
- **Ausencias deliberadas (si alguien las «arregla», rompe una garantía):** **contactos sin borrado** (la baja es exclusión reversible; en modo bot un borrado sería irreversible — misma razón que «Contactos NO tiene eliminar todos»); **difusiones y envíos por partes sin edición** (son el registro de lo que pasó: editarlos falsearía el historial y el comparador de precios); **auditoría sin edición**; **imágenes solo de escritura** (media referenciada por URL desde programados: borrarlas dejaría repetidos enviando enlaces rotos → ciclo de vida del bucket).
- **Nomenclatura normalizada (`entidad:accion`):** la clave de auditoría usa **exactamente** el segmento de la ruta HTTP como entidad y un verbo en infinitivo. Se renombraron 19 acciones heredadas (`user:*`→`users:*`, `config`→`config:guardar`, `destinatarios`→`patterns:guardar`, `queue_purge`→`queue:vaciar`, `dlq_redrive`→`dlq:reintentar`, `cancelar_envio`→`plans:cancelar`, `schedule:*`→`schedules:*`, `whatsapp_blocked_clear`→`whatsapp:reincluir`, `audit:limpiar`→`audit:borrar`…). El panel las traduce a español con **`ACC_ENT`** (entidad) + **`ACC_VRB`** (verbo) y deja la clave cruda en el `title`; `accLabel` devuelve tal cual las acciones antiguas sin `:` (los registros ya escritos en DynamoDB no se migran). **Al añadir una acción hay que tocar los tres sitios (ruta, `_audit`, etiqueta);** `NomenclaturaTests` recorre el AST del módulo y falla nombrando la acción incompleta.
- **Forma canónica de las rutas de escritura:** `POST /api/<entidad>/update` y `/delete`; los verbos propios se conservan cuando la operación **no** es un update genérico (`/role`, `/toggle`, `/cancel`, `/purge`, `/redrive`, `/clear`).
...
- **Vinculación guiada DESDE EL PANEL (mismo teléfono, sin scripts):** réplica del flujo del userbot de Telegram (número → código → conectado) en Ajustes → 🔌 Conexiones. Es **solo panel**: el servicio Node no cambió (`/pair` ya devuelve `pairingCode` y `/status` ya expone `connected`/`contacts`/`pairingCode`/`lastPairError`). Piezas en `admin.py`: segmentado `#wa_modo` (📱 *Desde este teléfono* por defecto / 🖥 *Con QR (otro aparato)*, el QR queda secundario), `waPair()`, `waSondear()` (poll de `/status` cada 3 s + re-poll inmediato en `visibilitychange`, que es la señal fiable de «volvió de WhatsApp»), `waPintarCodigo()` (formato `XXXX XXXX`), `waCopyCode()` (clipboard con fallback a selección), CSS `.wa-code-box/.wa-code/.wa-pasos/.wa-espera`. Detalles que importan: (1) **`/pair` es destructivo** (`clearOnStart=true` borra la sesión guardada) y el servicio devuelve **409 `ya_conectado`** si está conectado, así que el panel consulta `/status` primero, confirma con modal y **hace `/reset` antes** de pedir el código; (2) el sondeo se corta si el usuario sale de Conexiones (`!caja.offsetParent`), tras `lastPairError` o a los 5 min (`WA_MAX_ESPERA`, el código caduca); (3) al conectar **no recarga** (la credencial vive solo en memoria, M17): pinta ✅ + nº de contactos y refresca las pastillas; (4) los códigos de error del Node (`ya_conectado`/`numero_invalido`/`sin_codigo`/`whatsapp_no_configurado`/`whatsapp_inaccesible`) se traducen a castellano — `sin_codigo` explica el bloqueo a datacenters, sin fingir que fue culpa del usuario; (5) el número se recuerda en `localStorage.wa_num` (no es secreto) y la ruta `/api/whatsapp/pair` **audita `whatsapp:pair` con el número enmascarado** (solo los últimos 4 dígitos: la auditoría la ve cualquier usuario del panel) y normaliza a dígitos antes de llamar al servicio. Tests: `WhatsappPairTests` en `tests/test_entrypoint_admin.py`; captura visual `wa-vincular-*.png` de `scripts/capturas_ui.py`.
...
- **`/blocked/clear` acepta UN contacto (agosto 2026):** con `{id}` (jid completo o número pelado — se le añade `@s.whatsapp.net`) borra solo ese contador y responde `{ok, cleared, id}`; sin cuerpo sigue limpiando todos. **Ya desplegado** (Render auto-deploy, commit `329008b`); comprobable con `python scripts/verificar_deploy_render.py`.
- **Sello de build en el servicio (agosto 2026):** `/health` (público), `/status` y la página raíz publican `commit` (`RENDER_GIT_COMMIT`, 7 chars) y `src` — huella sha256 de `src/*.js` + `package.json` que el servicio calcula al arrancar. La huella no depende de variables del host (Fly/Koyeb/local no inyectan nada) y **normaliza CRLF** (el checkout de Windows los tiene y el de Linux no; sin normalizar, el mismo código daría huellas distintas). El panel la muestra en el estado de WhatsApp (`· build <sha>`). Antes de esto no había forma de distinguir «auto-deploy activo» de «llevo semanas con código viejo».
...
- **360 tests** pasando; 2 workflows CI (`tests.yml` + `deploy.yml`, deploy gated por `DEPLOY_ENABLED`).
...
- **CRUD completo por entidad + nomenclatura normalizada** (agosto 2026): editar programados y usuarios, renombrar listas arrastrando referencias, reincluir un solo auto-excluido de WhatsApp; acciones auditadas como `entidad:accion` alineadas con las rutas HTTP y traducidas al español en el panel. Matriz entidad × C/R/U/D en `docs/PLAN_PRUEBAS.md` §2.1; el detalle y las ausencias deliberadas, en «CRUD por entidad y nomenclatura de acciones» más arriba.
- **Documentación al día y LISTO PARA PRUEBAS FUNCIONALES:** manual de invitado (`docs/MANUAL_USUARIO_INVITADO.md`), `README.md`, plan de pruebas (`docs/PLAN_PRUEBAS.md`, con F1–F12 y la matriz CRUD §2.1) y este HANDOFF reflejan los cambios recientes (nombres sin jerga, buscador/paginación/eliminar-todos, borrar = desencolar, cola en vivo + vaciar, detalle enriquecido con mensaje anterior + comparador + fechas, matriz de estado, responsive, espaciado, **PWA + tema claro + nav inferior**, **CRUD por entidad**). Empezar por la checklist E2E de `docs/PLAN_PRUEBAS.md` §4 (F1–F12) con las cuentas de prueba y `sending_enabled=False` salvo el caso que lo pruebe.
