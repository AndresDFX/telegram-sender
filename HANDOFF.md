...
- **`/blocked/clear` acepta UN contacto (agosto 2026):** con `{id}` (jid completo o número pelado — se le añade `@s.whatsapp.net`) borra solo ese contador y responde `{ok, cleared, id}`; sin cuerpo sigue limpiando todos. **Pendiente de redeploy en Render:** el Lambda ya manda `{id}`, pero mientras el servicio sea el anterior lo ignora y limpia todos los contadores (superconjunto inocuo). Al desplegar, verificar F12.4 del plan de pruebas.
...
- **358 tests** pasando; 2 workflows CI (`tests.yml` + `deploy.yml`, deploy gated por `DEPLOY_ENABLED`).
